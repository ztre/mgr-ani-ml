...
def parse_tv_filename(filename: str) -> ParseResult | None:
    ...
    if se is not None:
        ...
        if kind in {"episode", "e", "number", "bracket", "two_digit"}:
            season_hint = _extract_season_keyword(episode_text) or _extract_roman_season(episode_text)
...
def _extract_roman_season(text: str) -> int | None:
    ...
    while True:
        ...
        s = s[:m.start()].strip()
    m = re.search(
        r"(?:\b(?:Season|S)\s*(II|III|IV|V|VI|VII|VIII|IX|X)\b|\b(II|III|IV|V|VI|VII|VIII|IX|X)\b)\s*$",
        s,
        re.I,
    )
    if not m:
...
def _extract_season_episode_priority(text: str) -> tuple[int, int, tuple[int, int], str] | None:
    ...
    if m:
        return 1, int(m.group(1)), m.span(), "bracket"
